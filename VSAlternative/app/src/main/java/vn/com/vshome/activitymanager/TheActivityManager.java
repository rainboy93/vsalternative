package vn.com.vshome.activitymanager;
...
public class TheActivityManager {
            ...
            public void onActivityResumed(Activity activity) {
                ...
                if (!(activity instanceof FullPreviewActivity)) {
                    if (CameraManager.getInstance().isPreviewing && Utils.isMyServiceRunning(PreviewService.class)) {
                        activity.sendBroadcast(new Intent(Define.INTENT_PREVIEW));
                    }
                }
            }
    ...
    public Application getApplication() {
        if (activities.size() > 0) {
            synchronized (activities) {
                return activities.get(0).getActivity().getApplication();
            }
        }
        return null;
    }
